...
async function createSVG (icons) {
	const promises = icons.map(({ shortName, source }) =>
		writeFile(`icons/${shortName}.svg`, source)
	);
...
async function createHTML (icons) {
	const promises = icons.map(async ({ shortName, source }) => {
		...
		await writeFile(`icons/${shortName}.html`, contents);
	});
	await Promise.all(promises);
}
...
async function createJS (icons) {
	const promises = icons.map(async ({ shortName, source }) => {
		...
		await writeFile(`icons/${shortName}.js`, contents);
	});
	await Promise.all(promises);
}
...
async function createBundle (icons) {
	const allIcons = icons
		.map(({ shortName, source }) => `registerIcon('${shortName}', \`${source}\`);\n`)
		.join('');
	const contents =
...
async function createJSON (icons) {
	const data = icons.map(({ shortName }) => shortName)
