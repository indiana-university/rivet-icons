...
const { ICON_BUILD_DIR, ICON_BUILD_FILE_NAME, ICON_NAMESPACE } = require('./constants.js')
...
`
  ...
  const renameIcons = icons.map(({ fullFileName, source }) =>
    fs.writeFile(path.resolve(ICON_BUILD_DIR, fullFileName), source)
  )
  ...
  const inlineIcons = icons.map(({ fullName, source }) => {
    const svg = source
      ...
      .replace(/</g, '  <')
      .replace(/\n$/, '')
    const contents = `<span class="rvt-icon">\n${svg}\n</span>\n`
    return fs.writeFile(path.resolve(ICON_BUILD_DIR, `${fullName}.html`), contents)
  })
