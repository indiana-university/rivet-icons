var H=(s,v,h)=>{if(!v.has(s))throw TypeError("Cannot "+h)};var g=(s,v,h)=>(H(s,v,"read from private field"),h?h.call(s):v.get(s)),i=(s,v,h)=>{if(v.has(s))throw TypeError("Cannot add the same private member more than once");v instanceof WeakSet?v.add(s):v.set(s,h)},d=(s,v,h,l)=>(H(s,v,"write to private field"),l?l.call(s,h):v.set(s,h),h);var V=(s,v,h)=>(H(s,v,"access private method"),h);(function(s){typeof define=="function"&&define.amd?define(s):s()})(function(){var w,c,e,a,Z,f,p,C;"use strict";const s="rvt-icon",v="name",h="rvtIconRegistered",l=new Map,M=new Map,x=document.createElement("style");x.setAttribute(`data-${s}`,""),document.head.appendChild(x);const L=x.sheet;function t(n,r){const o="Rivet Icon";if(!n||typeof n!="string")throw new Error(`${o}: Name must be a string.`);const m=document.createElement("template");if(m.innerHTML=r,m.content.children.length!==1)throw new Error(`${o} (${n}): Content must contain one SVG element.`);if(m.content.firstChild.nodeName.toLowerCase()!=="svg")throw new Error(`${o} (${n}): Content must be a SVG element.`);l.set(n,m);const $=l.size;M.set($,n),L.insertRule(`${s} { --${n}: ${$}; }`);const b=new CustomEvent(h,{detail:{name:n}});document.dispatchEvent(b)}const u=document.createElement("template");u.innerHTML=`
	<style>
		:host,
		.container {
			display: inline-flex;
